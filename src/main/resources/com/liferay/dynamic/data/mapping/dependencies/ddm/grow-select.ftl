<#-- Keeps all render logic which configured when creating the field-->
<#-- Tag libraries -->
...
<script data-senna-track="temporary" type="text/javascript">
    CKEDITOR.on('instanceReady', function (event) {
        if (event.editor.name.includes('${fieldStructure.editorName}')) {
            var growEditor = CKEDITOR.instances[event.editor.name];

            var growSelect = AUI().one('.${namespacedFieldName}');

            if (growSelect.getDOMNode().value == 'markdown' && growEditor.mode == 'wysiwyg') {
                growEditor.getCommand('markdown').setState(CKEDITOR.TRISTATE_DISABLED);
                growEditor.setMode('markdown');
            }

            growSelect.on('change', function(e) {
                if (growEditor) {
                    if (growEditor.mode == 'wysiwyg') {
                        growEditor.fire('saveSnapshot');
                    }
                    if (growSelect.getDOMNode().value == 'markdown' &&
                        growEditor.mode == 'wysiwyg'
                        ||
                        growSelect.getDOMNode().value == 'html' &&
                        growEditor.mode == 'markdown') {
                            growEditor.getCommand('markdown').setState(CKEDITOR.TRISTATE_DISABLED);
                            growEditor.setMode(growEditor.mode == 'markdown' ? 'wysiwyg' : 'markdown');
                    }
                }
            });

             var clearEventHandlers = function(event) {
                Liferay.detach('change', growSelect);
                growEditor = null;
                growSelect = null;
            };

            Liferay.once('destroyPortlet', clearEventHandlers);
        }
    });
</script>